...
```
frontend/src/
├── views/HomeView.vue            # 主容器（3D翻转登录/注册 + 顶部悬浮导航 + 面板切换 + 像素背景动画）
├── components/
...
| 特性 | 说明 |
|------|------|
| 登录页 | 3D 翻转卡片（rotateY 180°），滑块 + 文字链接切换登录/注册，蓝色像素下落背景 + 鼠标交互（避让+发光） |
| 主界面 | 顶部悬浮毛玻璃导航栏（blur 16px），蓝色像素下落背景（无鼠标交互），内容区毛玻璃卡片 |
...
| 2026-07-20 | 暗色根因修复：`:root` 覆盖 Element Plus CSS 变量(20+个)，+ `.el-select__wrapper` 适配 2.4+ 新结构 |
...
| 2026-07-20 | 暗色下拉菜单/消息提示/表单标签/表格，Popconfirm 与 Select 弹出层暗色适配 |
| 2026-07-20 | 控制台卡片导航：四张统计卡片点击跳转四大管理模块，侧边栏精简为顶栏 |
